...
abstract class Mage_Core_Block_Abstract extends Varien_Object
{
    ...
    final public function toHtml()
    {
        $helperBrCore = null;
        $blockId = null;
        $switcher = ($helperBrCore = Tommy_DebugInfo_Helper_Data::getMe()) && self::$_switcher;
        if ($switcher) {
            $blockId = (Tommy_DebugInfo_Helper_Data::$_i++);
            $this->setData('debug-info-block-id', $blockId);
        }
        ...
        if ($switcher) {
            $helperBrCore->addPerformanceLog('load from cache', $blockId);
        }
        $html = $this->_loadCache();
        if ($switcher) {
            $helperBrCore->addPerformanceLog('load from cache', 'fin_' . $blockId);
        }
        if ($loadedFromCache = $html === false) {
            ...
            if ($switcher) {
                $helperBrCore->addPerformanceLog('overall', $blockId);
            }
            $this->_beforeToHtml();
            $html = $this->_toHtml();
        ...
        if ($switcher) {
            $helperBrCore->addPerformanceLog('overall', 'fin_' . $blockId);
            $children = array();
            foreach ($this->_children as $alias => $block) {
                $children[$alias] = $block->getData('debug-info-block-id');
            }
            $outDebug = array(
            ...
            if ($cacheId = $this->getData('debug_info_from_cache')) {
                $outDebug['cacheId'] = $cacheId;
            }
            if ($parentBlock = $this->getParentBlock()) {
                $outDebug['parentId'] = $parentBlock->getData('debug-info-block-id');
            }
            Tommy_DebugInfo_Helper_Data::$_blocksInfo[$blockId] = $outDebug;
        }
        $loadedFromCache = !$loadedFromCache;
        if ($loadedFromCache && $switcher && $helperBrCore->getDebugCompareHtml()) {
            $this->_beforeToHtml();
            $htmlCopy = $this->_toHtml();
            if ($htmlCopy != $html) {
                $helperBrCore->addDebugOutput('compare',
                                              $this->getNameInLayout() . '(' . $blockId . ') not equal',
                                              $blockId,
                                              'strict');
                //todo: remove it!!
                file_put_contents('/home/tomfun/prj/glazikm/some.html', $html);
                file_put_contents('/home/tomfun/prj/glazikm/someCopy.html', $htmlCopy);
            }
        }
        if ($switcher && $loadedFromCache && $cacheId) {
            $helperBrCore->addDebugOutput('cache keys',$cacheId, $blockId);
            $helperBrCore->addPerformanceLog('afterToHtml', $blockId);
        }
        $html = $this->_afterToHtml($html);
        if ($switcher) {
            $helperBrCore->addPerformanceLog('afterToHtml', 'fin_' . $blockId);
        }
        if ($switcher) {
        ...
        if ($switcher && $loadedFromCache && $helperBrCore->getDebugCustomHints()) {
            $html = '<div class="debug-info-container" data-saved-cache-id="'
                . $this->getData('debug_info_save_cache')
                . '" data-cache-id="'
                . $this->getData('debug_info_from_cache')
                . '"'
                . 'data-template="'
                . $this->getTemplate()
                . '"'
        ...
        if ($switcher) {
            $helperBrCore->addPerformanceLog('abstract_to_html_after+', $blockId);
        }
        ...
        if ($switcher) {
            $helperBrCore->addPerformanceLog('abstract_to_html_after+', 'fin_' . $blockId);
        }
    ...
    protected function _loadCache()
    {
        ...
        $this->setData('debug_info_from_cache', $cacheKey);
        return $cacheData;
    }
    ...
    protected function _saveCache($data)
    {
        ...
        $this->setData('debug_info_save_cache', $cacheKey);
        return $this;
    }
